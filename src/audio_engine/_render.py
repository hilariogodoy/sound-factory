...
try:
    ...
    # --- Output mix ---
    mixer = Mixer(outs=2, chnls=1)
    mixer.addInput(0, kick_sig)
    mixer.addInput(0, hat_sig)
    mixer.addInput(0, bass_filt)
    mixer.addInput(0, pad)
    mixer.setAmp(0, 0, 0.7)
    mixer.setAmp(1, 0, 0.5)
    mixer.setAmp(2, 0, 0.6)
    mixer.setAmp(3, 0, 0.4)
    stereo_out = mixer.mix(2).out()
